...
class _EditApplianceScreenState extends State<EditApplianceScreen> {
  ...
  void initState() {
    super.initState();
    // Vul de controllers met bestaande gegevens
    _titleController = TextEditingController(text: widget.appliance.title);
    _descriptionController = TextEditingController(
      text: widget.appliance.description,
    );
    _priceController = TextEditingController(
      text: widget.appliance.pricePerDay.toString(),
    );
    _addressController = TextEditingController(
      text: widget.appliance.address ?? '',
    );
    _selectedCategory = widget.appliance.category;
    _existingImageUrl = widget.appliance.imageUrl;
    _isAvailable = widget.appliance.isAvailable;
    _latitude = widget.appliance.latitude;
    _longitude = widget.appliance.longitude;
    _loadUserName();
  }
      ...
      try {
        ...
        if (user == null) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Je moet ingelogd zijn om een apparaat te bewerken',
              ),
            ),
          );
          setState(() {
        ...
        final updatedAppliance = Appliance(
          ...
          imageUrl: imageUrl,
          createdAt: widget.appliance.createdAt,
        ...
        await _applianceService.updateAppliance(
          widget.appliance.id!,
          updatedAppliance.toMap(),
        );
        debugPrint('Apparaat bijgewerkt met ID: ${widget.appliance.id}');

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Apparaat succesvol bijgewerkt')),
        );

        Navigator.pop(context);
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Fout bij bewerken apparaat: $e')),
        );
      } finally {
        setState(() {
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Apparaat Bewerken')),
      body:
          _isLoading
